...
class UsersServiceImpl implements UsersService {
    public refreshJwtToken = async (
        ...
        const sql = {
            selectToken: `SELECT user_id AS userId, access_ip AS accessIp, expire_date AS expireTime, token
        ...
        if (!dbToken)
            throw new NoDataError('No session found for this token - please, login first!');
        ...
        if (new Date(expireDate).getTime() < new Date().getTime()) {
            const dbDeletionResponse = await sqlPool.query(sql.deleteToken);
            log.debug(dbDeletionResponse);
            throw new NoDataError('Session was expired - please, login again!');
        }
